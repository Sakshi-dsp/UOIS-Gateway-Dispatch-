...
func TestIssueStatusHandler_HandleOnIssueStatus_Success(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	reqBody := map[string]interface{}{
		...
		"message": map[string]interface{}{
			"issue": map[string]interface{}{
				"id":     "issue-123",
				"status": "CLOSED",
				"issue_actions": map[string]interface{}{
					"respondent_actions": []interface{}{
	...
	mockRepo.On("GetIssue", mock.Anything, "issue-123").Return(existingIssue, nil)
	mockRepo.On("StoreIssue", mock.Anything, mock.Anything).Return(nil)

	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest("POST", "/on_issue_status", nil)

	c.Request.Header.Set("Content-Type", "application/json")
	bodyBytes, _ := json.Marshal(reqBody)
	c.Request.Body = http.MaxBytesReader(w, httptest.NewRequest("POST", "/on_issue_status", bytes.NewReader(bodyBytes)).Body, 1048576)

	handler.HandleOnIssueStatus(c)

	assert.Equal(t, http.StatusOK, w.Code)
	mockRepo.AssertExpectations(t)
}

func TestIssueStatusHandler_HandleOnIssueStatus_IssueNotFound(t *testing.T) {
	gin.SetMode(gin.TestMode)
