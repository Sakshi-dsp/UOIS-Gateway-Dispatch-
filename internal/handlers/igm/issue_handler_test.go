...
func TestIssueHandler_HandleOnIssue_Success(t *testing.T) {
	...
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest("POST", "/on_issue", nil)

	c.Request.Header.Set("Content-Type", "application/json")
	bodyBytes, _ := json.Marshal(reqBody)
	c.Request.Body = http.MaxBytesReader(w, httptest.NewRequest("POST", "/on_issue", bytes.NewReader(bodyBytes)).Body, 1048576)

	handler.HandleOnIssue(c)

	assert.Equal(t, http.StatusOK, w.Code)
	mockRepo.AssertExpectations(t)
}

func TestIssueHandler_HandleOnIssue_IssueNotFound(t *testing.T) {
	gin.SetMode(gin.TestMode)
