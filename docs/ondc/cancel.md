# cancel API (Seller NP)

## 1. Overview
- Purpose of this API: This API allows Buyer NP to request cancellation of an existing logistics order
- Who calls it (Buyer NP / Seller NP): Called by Buyer NP (BAP)
- Sync vs Async behavior: Asynchronous - Buyer NP sends request, Seller NP responds with ACK/NACK synchronously, then sends callback via /on_cancel
- Callback expectations: Seller NP MUST send /on_cancel callback with cancellation confirmation within TTL window
...
- We receive cancellation requests for existing orders
- We validate the cancellation reason against our terms
- We determine if cancellation is allowed based on fulfillment state
- We calculate any applicable cancellation fees
- We return ACK/NACK synchronously
- We send cancellation confirmation asynchronously via /on_cancel callback
...
| context.action | string | Y | Action name (cancel) | Buyer NP |
...
| message.order_id | string | Y | Order identifier to cancel | Buyer NP |
| message.cancellation_reason_id | string | Y | Cancellation reason code | Buyer NP |
...
| message.order.cancellation.cancelled_by | string | Y | Who cancelled the order | Seller NP |
...
| message.order.quote | object | Y | Final quote with any applicable fees | Seller NP |
| message.order.fulfillments | array | Y | Fulfillment details | Seller NP |
...
- message_id: Unique identifier for this cancel request
...
## 8. Validation Rules
- Mandatory field checks: order_id and cancellation_reason_id must be present and valid
- Cancellation validation: Reason code must be valid and allowed for current fulfillment state
- TAT validation: For TAT breach cancellations (reason "007"), verify actual vs promised TAT
...
|----------|------------|----------------|------------------|
| Invalid cancellation reason | 60009 | Reason code not valid | Send NACK with LSP-ERROR |
| TAT not breached | 60010 | Cancellation reason 007 but TAT not breached | Send NACK with LSP-ERROR |
...
- Seller NP MUST validate cancellation reason against current fulfillment state
- Seller NP MUST check TAT breach claims for reason code "007"
...
- Seller NP MUST update cancellation.cancelled_by appropriately
- Seller NP SHOULD include IGM request reference if applicable
- Common mistake to avoid: Accepting invalid cancellation reasons or not validating TAT breaches
