...
## 5. ID Domain Isolation & Echo Contract Pattern (CRITICAL)
...
Seller IDs can be sent to Buyer NP only as opaque values in ONDC-allowed fields, and Buyer NP must treat them as black boxes.

- Buyer does not interpret

- Buyer does not generate

Buyer only echoes when protocol requires
- **ID mapping, derivation, and inference are strictly forbidden** in UOIS Gateway.
...
- **ID Propagation via Echo Contract**:
  - UOIS Gateway manages internal processing identifiers by generating or receiving them from owning services
  - UOIS Gateway propagates internal IDs to Buyer NPs inside ONDC-compliant opaque fields
  - Buyer NPs are required to echo these identifiers unchanged in subsequent requests
  - UOIS Gateway extracts echoed identifiers only from predefined protocol locations
  - IDs are treated as opaque tokens owned by the generator, echoed by contract

- **Echo Contract Table**:

| Internal ID | Generated By | Where UOIS Sends It | Where Buyer NP Must Echo It | Extraction Location |
|-------------|-------------|---------------------|----------------------------|---------------------|
| `search_id` | UOIS Gateway | `/on_search` → `message.catalog.providers[].id` | `/init` → `message.order.provider.id` | `message.order.provider.id` |
| `quote_id` | Order Service | `/on_init` → `message.order.quote.id` | `/confirm` → `message.order.quote.id` | `message.order.quote.id` |
| `dispatch_order_id` | Order Service | `/on_confirm` → `message.order.id` | N/A (not echoed; internal only) | N/A |

- **ID Ownership Rules**:
  - UOIS Gateway **generates only IDs it owns** (e.g., search_id, trace_id, event_id)
  - Internal service IDs are **accepted only from owning services** (quote_id from Order Service, dispatch_order_id from Order Service)
  - **Client-provided IDs (`client_order_id`)**:
    - **Who creates it**: Client (Buyer NP)
    - **Where it originates**: ONDC payload (`message.order.id` in `/confirm` request)
    - **Stability**: Stable across lifecycle
    - **Semantic meaning**: Client's business reference (not a protocol-only ID like transaction_id/message_id)
    - **Ownership**: Client-owned, external to UOIS
    - **Flow requirement**: MUST be passed to internal systems (Order Service, etc.) unchanged
    - **Internal use-cases** (non-negotiable):
      - Callback composition: `/on_status`, `/on_track`, `/on_cancel` MUST return `order_id` (client_order_id)
      - Audit & dispute resolution: Support teams reference client order IDs
      - IGM/grievance flows: Issues raised against client order references
      - Customer support tooling: Search by client order ID
      - Reconciliation: Matching client invoices vs internal orders
      - Rider App / Ops UI: Client reference shown alongside dispatch ID
    - **Not subject to echo contract**: `client_order_id` is provided by client, not generated by UOIS, so it does not follow echo contract pattern

- **Fail-Fast Behavior**:
  - If an expected echoed ID is missing from the predefined protocol location → return NACK immediately
  - If an ID was not previously issued by UOIS or an owning service → return NACK immediately
  - No fallback resolution, no inference, no guessing

- **Order Record Storage**:
  - UOIS Gateway stores multiple identifiers on the same order record; it does not translate, map, derive, or resolve identifiers across domains
  - Order record stores: `client_order_id` (external business reference), `dispatch_order_id` (internal execution identifier), `quote_id`, `search_id`
  - Multiple identifiers coexist on same order record; no identifier replaces or represents another
  - Lookup is always performed by primary identifier expected by the API. No identifier is inferred, derived, or substituted

- Any code that:
  - derives an ID from payload fields
  - reuses protocol IDs as business IDs
  - assumes equivalence across domains
  - attempts fallback resolution when expected IDs are missing
